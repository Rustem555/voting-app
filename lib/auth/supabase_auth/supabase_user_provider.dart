...
class VotingMonoLocalPushDelSupabaseUser extends BaseAuthUser {
  VotingMonoLocalPushDelSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> votingMonoLocalPushDelSupabaseUserStream() {
  ...
  return (!loggedIn
    ...
    (authState) {
      currentUser =
          VotingMonoLocalPushDelSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
